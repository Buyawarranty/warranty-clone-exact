...
SET 
  monthly_price = CASE 
    ...
    WHEN name = 'Gold' THEN 27.00  
    WHEN name = 'Platinum' THEN 29.00
    ELSE monthly_price
  END,
  yearly_price = CASE 
    WHEN name = 'Basic' THEN 540.00
    WHEN name = 'Gold' THEN 583.00
    WHEN name = 'Platinum' THEN 626.00
    ELSE yearly_price
  END,
  three_yearly_price = CASE 
    WHEN name = 'Basic' THEN 792.00
    WHEN name = 'Gold' THEN 855.00
    WHEN name = 'Platinum' THEN 919.00
...
SET 
  monthly_price = CASE 
    WHEN vehicle_type = 'PHEV' THEN 27.00
    WHEN vehicle_type = 'EV' THEN 27.00  
    WHEN vehicle_type = 'MOTORBIKE' THEN 25.00
    ELSE monthly_price
  END,
  yearly_price = CASE 
    WHEN vehicle_type = 'PHEV' THEN 583.00
    WHEN vehicle_type = 'EV' THEN 583.00
    WHEN vehicle_type = 'MOTORBIKE' THEN 540.00
    ELSE yearly_price
  END,
  three_yearly_price = CASE 
    WHEN vehicle_type = 'PHEV' THEN 855.00
    WHEN vehicle_type = 'EV' THEN 855.00
    WHEN vehicle_type = 'MOTORBIKE' THEN 792.00
    ELSE three_yearly_price
  END,
  pricing_matrix = CASE 
